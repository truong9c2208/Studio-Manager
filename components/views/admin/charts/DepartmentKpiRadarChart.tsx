...
interface DepartmentKpiRadarChartProps {
    members: Employee[];
}

export const DepartmentKpiRadarChart: React.FC<DepartmentKpiRadarChartProps> = ({ members }) => {
    const averageKpi = useMemo<KPI>(() => {
        ...
        if (members.length === 0) return kpiTotals;
        ...
        Object.keys(kpiTotals).forEach(key => {
            kpiTotals[key as keyof KPI] = Math.round(kpiTotals[key as keyof KPI] / members.length);
        });
    ...
    return (
        <Card title="Average Department KPIs">
            <div className="flex items-center justify-center h-64">
                 {members.length > 0 ? (
                    ...
                    <p className="text-text-secondary">No members in this department to calculate KPIs.</p>
                 )}
            </div>
        </Card>
    );
