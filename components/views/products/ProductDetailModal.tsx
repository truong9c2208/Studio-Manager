...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({ isOpen, onClose, product, category, invoices, onEdit, onDelete, isAdmin }) => {
    ...
    return (
        ...
        >
            <div 
                className="bg-slate-800 border border-slate-600 rounded-lg shadow-xl w-full m-4 max-w-4xl flex flex-col"
            ...
            >
                ...
                <div className="flex justify-between items-center p-4 border-b border-secondary">
                  ...
                  <button onClick={onClose} className="text-white hover:text-text-primary">
                ...
                <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
                    <header className="flex items-start space-x-6">
                        ...
                        <div className="flex-grow">
                            ...
                            <p className="text-sm font-mono text-white mt-1">SKU: {product.sku}</p>
                            <div className="mt-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div className="flex items-center p-3 bg-slate-500 rounded-lg">
                            <DollarSignIcon className="w-6 h-6 mr-3 text-pink-300" />
                            <div>
                                <p className="text-white">Price</p>
                                <p className="font-bold text-lg text-sky-300">${product.price.toFixed(2)}</p>
                            </div>
                        </div>
                        <div className="flex items-center p-3 bg-slate-500 rounded-lg">
                            <ArchiveBoxIcon className="w-6 h-6 mr-3 text-pink-300" />
                            <div>
                                <p className="text-white">Stock</p>
                                <p className="font-bold text-lg text-sky-300">{product.stock} units</p>
                            </div>
                        </div>
                        <div className="flex items-center p-3 bg-slate-500 rounded-lg">
                            <CheckCircleIcon className="w-6 h-6 mr-3 text-pink-300" />
                            <div>
                                <p className="text-white">Status</p>
                                <p className="font-bold text-lg text-sky-300">
                    ...
                    <section>
                        <h3 className="font-semibold text-lg mb-2 text-white">Recent Sales</h3>
                ...
                {/* Footer for Actions */}
                <div className="flex justify-between items-center p-4 bg-slate-800 border-t border-primary rounded-b-lg">
                    {isAdmin ? (
                        <button 
